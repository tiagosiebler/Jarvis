		...
		"attachments": [
			{
				...
				"color": "#36a64f",
				"author_name": searchResults.total + " search results from the "+section+", top results:",
...
var triggerSearchInSection = function(bot, message, section, sectionConst, keywords){
	scraper.scrapeDevZoneResults(sectionConst, keywords, 4, function(result){
		if(!result.success){
			...
			if(result.error.message.indexOf("wait() timed out after") != -1){
				...
				bot.reply(message, generatePlainAttachmentStrWithLink("Search for keywords failed: "+keywords,result.url,"Either no results were returned or the search timed out. Click the title of this message to try it yourself."));
			}else{
				// unknown error, try direct URL: result.url
				bot.reply(message, generatePlainAttachmentStrWithLink("Search for keywords failed: "+keywords,result.url,"An unknown error happened during the search. Click the title of this message to try it yourself."));
			}
		}else{
...
var triggerSearchInSectionSwitch = function(bot, message, section, keywords){
	...
	switch(section){
		...
		case "Data Connector SDK DevZone":
			triggerSearchInSection(bot, message, section, "DataConnectorSDK", keywords);
			break;	
	}
}
var triggerSearch = function(bot, message, params) {
	...
	if (searchLocation.match(regexList["websdk"])) {
		bot.startConversationInThread(message, function(err, convo) {
			if (!err) {
				convo.say("Searching Web SDK documentation using keywords (" + searchKeywords.join(" ") + ")");
				convo.activate();

				triggerSearchInSectionSwitch(bot, message, "Web SDK DevZone", searchKeywords.join(" "));
			}
		});

	} else if (searchLocation.match(regexList["mobilesdk1"]) || searchLocation.match(regexList["mobilesdk2"]) || searchLocation.match(regexList["mobilesdk3"])) {
		bot.startConversationInThread(message, function(err, convo) {
			if (!err) {
				convo.say("Searching Mobile SDK documentation using keywords (" + searchKeywords.join(" ") + ")");
				convo.activate();
				
				triggerSearchInSectionSwitch(bot, message, "Mobile SDK DevZone", searchKeywords.join(" "));
			}
		});
		...
		bot.startConversationInThread(message, function(err, convo) {
			if (!err) {
				convo.say("Searching Visualization SDK documentation using keywords (" + searchKeywords.join(" ") + ")");
				convo.activate();

				triggerSearchInSectionSwitch(bot, message, "Visualization SDK DevZone", searchKeywords.join(" "));
			}
		});

	} else if (searchLocation.match(regexList["restapi1"]) || searchLocation.match(regexList["restapi2"]) || searchLocation.match(regexList["restapi3"]) || searchLocation.match(regexList["restapi4"])) {
		bot.startConversationInThread(message, function(err, convo) {
			if (!err) {
				convo.say("Searching REST JSON Data API documentation using keywords (" + searchKeywords.join(" ") + ")");
				convo.activate();

				triggerSearchInSectionSwitch(bot, message, "REST API DevZone", searchKeywords.join(" "));
			}
		});

	} else if (searchLocation.match(regexList["datasdk1"]) || searchLocation.match(regexList["datasdk2"]) || searchLocation.match(regexList["datasdk3"])) {
		bot.startConversationInThread(message, function(err, convo) {
			if (!err) {
				convo.say("Searching Data Connector SDK documentation using keywords (" + searchKeywords.join(" ") + ")");
				convo.activate();

				triggerSearchInSectionSwitch(bot, message, "Data Connector SDK DevZone", searchKeywords.join(" "));
			}
		});

	} else if (searchLocation.match(regexList["kbase1"]) || searchLocation.match(regexList["kbase2"]) || searchLocation.match(regexList["kbase3"]) || searchLocation.match(regexList["kbase4"])) {
		bot.startConversationInThread(message, function(err, convo) {
			if (!err) {
				convo.say("Searching Knowledge Base using keywords (" + searchKeywords.join(" ") + ")");
		...
		bot.startConversationInThread(message, function(err, convo) {
			if (!err) {
				convo.say("Unrecognized SDK search, please specify which section to search, e.g: websdk, mobilesdk, vissdk, restapi or datasdk (data connector).");
				convo.activate();
			}
		});
		...
		bot.startConversationInThread(message, function(err, convo) {
			if (!err) {
				convo.say("Unrecognized search location, please specify which location to search, e.g: websdk, mobilesdk, vissdk, restapi, datasdk (data connector), kbase.");
				convo.activate();
			}
		});
	}
...
module.exports = function(controller) {
	...
	controller.hears(['(.*)!search(.*)'], listenScope["everywhere"], function(bot, message) {
		...
		switch (params.length) {
			case 1:
				triggerQuickLink(bot, message, "The syntax is !search <location> <keywords>. Example locations are: websdk, mobilesdk, vissdk, datasdk, restapi. More can be added by request.");
				break;

			case 2:
				triggerQuickLink(bot, message, "Missing keywords (3rd parameter). The expected syntax is !search <location> <keywords>, you said: " + message.text);
				break;
	...
	controller.hears(['asdfadsfasfasfasdfadsf'], listenScope["everywhere"], function(bot, message) {
