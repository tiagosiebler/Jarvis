...
pool.query("SELECT 'stuff'", (error, results, fields) => {
  ...
  if (results[0].stuff != "stuff") return console.error("ERROR: extDB sanity check failed");
...
class ExtDB {
  ...
  */
  setSFThreadForSlackThread(controller, message, sf_case, sf_post_id, shouldSync, callback) {
    ...
    flow.exec(
      ...
      function(results) {
        ...
        if (!isKnownUser) {
          ...
          if (!userInfo || !userInfo[1].user || !userInfo[1].user.profile.email) {
            const cleanedUserInfo = (userInfo && JSON.stringify(userInfo, null, 4)) || null;

            // !!userInfo[1].user, !!userInfo[1].user.profile.email
            console.warn('missing user info: (cleaned, userInfoDefined, shouldRefrehs, isKnownUser): ', cleanedUserInfo, !!userInfo, shouldRefresh, isKnownUser);

            if (userInfo && userInfo[1].ok) {
              console.log('userInfo1user, userinfo1.user.profile.email', !!userInfo[1].user, userInfo[1].user.profile.email);
            } else {}
            return;
          }
