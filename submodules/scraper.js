...
Scraper.prototype.isQuestionOnVersionSupport = function(message, callback) {
  ...
  if (
    stringContains(message, 'supported on') ||
    stringContains(message, 'iOS') ||
    stringContains(message, 'ios')
  ) {
...
Scraper.prototype.getExpirationForVersion = function(version, callback) {
  ...
  url =
    'https://www.microstrategy.com/us/services/customer-support/expiration-schedule';
  ...
  request(url, function(error, response, html) {
    if (!error) {
      ...
      var matchingVersion = $(
        '#schedule .alternating-rows tr td strong:contains(' + version + ')'
      );
      ...
      if (matchingVersion.length == 0) {
        ...
        var result = {
          ...
          errorMSG:
            ...
            '). Searching for ' +
            version +
            " wasn't specific enough, or an error occurred. Versions should be provided uniquely, e.g: 10.4",
          errorID: 2,
          url: url
        };
        callback(result);
...
var cleanMSDLResults = function(dom, callback, urlPrefix, url) {
  var totalSpan = dom.window.document.getElementsByClassName(
    'total-results'
  )[0];
  ...
  for (i = 0; i < maxResults && i < searchResults.children.length; i++) {
    processedResults[i] = {
      link:
        urlPrefix +
        encodeURI(
          searchResults.children[i].children[0].children[0].attributes[0].value
        ),
...
Scraper.prototype.scrapeDevZoneResults = function(
  section,
  query,
  maxCount,
  callback
  ...
  maxResults = maxCount;
  var urlPrefix =
    'https://lw.microstrategy.com/msdz/MSDL/GARelease_Current/docs/projects/' +
    section +
    '/Content/Search.htm?';
...
Scraper.prototype.getReadmeList = function(callback) {
  ...
  nightmare
    ...
    .evaluate(function() {
      var readmeContainer = document.getElementById(
        'MicroStrategyProductDocumentation-ReadmeandReleaseNotesbyVersion'
      ).parentNode;
