...
pool.query("SELECT 'stuff'", (error, results, fields) => {
  ...
  if (results[0].stuff != 'stuff')
    console.error('ERROR: extDB sanity check failed');
  else {
...
var generateInsertPost = function(
  messageTS,
  threadTS,
  messageText,
  postURL,
...
*/
ExtDB.prototype.setSyncStateForSlackThread = function(
  controller,
  message,
  shouldSync,
  callback
...
*/
ExtDB.prototype.setSFThreadForSlackThread = function(
  controller,
  message,
  sf_case,
  sf_post_id,
...
ExtDB.prototype.getSFThreadForSlackThread = function(
  controller,
  message,
  callback
) {
...
ExtDB.prototype.lookupChannel = function(bot, message, callback) {
  ...
  flow.exec(
    function() {
      if (false)
        console.log('ExtDB.lookupChannel() checking for known channel');
    ...
    function(results) {
      ...
      if (results.lookup_channel[0]) {
        ...
        if (false)
          console.log(
            'WARNING: ExtDB.lookupChannel error in SQL query: ',
            results.lookup_channel[0]
          );
        callback(err, null);
        return;
      } else {
        ...
        if (typeof queryResults == 'undefined') {
          if (false)
            console.log(
              "ERROR ExtDB.lookupChannel(): couldn't connect to DB properly: ",
              results.lookup_channel
            );
          return callback(results.lookup_channel, null, null);
        }
        if (queryResults.length == 0) {
          if (false)
            console.log('ExtDB.lookupChannel() channel not known in lookup');
        } else {
          if (false)
            console.log('ExtDB.lookupChannel() channel known in lookup');
          isKnownChannel = true;
        ...
        if (shouldRefresh || !isKnownChannel) {
          if (false)
            console.log(
              'ExtDB.lookupChannel() executing channel lookup via slack'
            );
    ...
    function(results) {
      ...
      if (!isKnownChannel) {
        if (false)
          console.log(
            'ExtDB.lookupChannel() channel not known, preparing output to save, ',
            channelInfo[1]
          );
        ...
        if (typeof channelInfo[1].channel == 'undefined') {
          ...
          SQLpost = {
            ...
            slack_channel_visibility: channelInfo[1].channel.is_private
              ? 'Private'
              : 'Public',
            dt_last_resolved: new Date()
          };
        ...
        if (shouldRefresh) {
          ...
          SQLstatement =
            'UPDATE ?? SET ? WHERE slack_channel_id_int = ' +
            SQLpost.slack_channel_id_int;
          delete SQLpost.slack_channel_id_int;

          SQLpost.slack_channel_name = channelInfo[1].channel.name;
          SQLpost.slack_channel_visibility = channelInfo[1].channel.is_private
            ? 'Private'
            : 'Public';
          SQLpost.dt_last_resolved = new Date();
          ...
          if (false)
            console.log(
              'ExtDB.lookupChannel() channel known, preparing output to update ',
              SQLpost
            );
        } else {
          if (false)
            console.log(
              'ExtDB.lookupChannel() have all channel info, just blindly calling back: ',
              channelInfoLU[0]
            );
      ...
      if (runSQL) {
        if (false)
          console.log(
            'ExtDB.lookupChannel() running channel SQL: ',
            SQLstatement
          );

        pool.query(
          SQLstatement,
          [process.env.mysqlTableChannelsLU, SQLpost],
          this.MULTI('channelLU_SQL')
        );
      }
    ...
    function(results) {
      ...
      if (false)
        console.log(
          'ExtDB.lookupChannel() running last block callback with: ',
          channelInfo
        );
...
ExtDB.prototype.lookupUserAndChannel = function(
  controller,
  bot,
  message,
  callback
  ...
  flow.exec(
    function() {
      if (false)
        console.log('lookupUserAndChannel() calling user and channel lookups');
    ...
    function(results) {
      if (false)
        console.log('lookupUserAndChannel() results received: ', results);
      ...
      if (false)
        console.log(
          'lookupUserAndChannel() results received without error, running parent callback: ',
          user[1],
          channel[1]
        );
...
*/
ExtDB.prototype.handleFailedSyncComment = function(
  failedSOQL,
  errorInfo,
  messageTS,
  threadTS
